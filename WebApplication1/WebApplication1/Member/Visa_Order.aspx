<%@ Page Title="" Language="C#" MasterPageFile="~/Member/Member.Master" AutoEventWireup="true" CodeBehind="Visa_Order.aspx.cs" Inherits="TotCms.Web.Member.Visa_Order" %>
...
<asp:Repeater ID="RepeatList" runat="server" OnItemCommand="RepeatList_ItemCommand">
        ...
        <ItemTemplate>
          <tr class="lines">
            ...
            <td>
              <div align="left">&nbsp;<a href="/Visa/Show.aspx?id=<%# Eval("VisaId") %>" target="_blank"><%# Eval("Title")%></a></div>
            </td>       
            <td><%# FormatDate(Eval("GoDate"))%></td>  
            <td><%# Eval("Price")%></td>              
            <td><%# Convert.ToInt32(Eval("PaySts")) == 1 ? "已支付" : "未支付 <a class=\"red\" href=\"Visa_Pay.aspx?id=" + Eval("id") + "\">支付？</a>"%></td>          
            <td><%# GetSts(Eval("OrderSts"))%></td>                  
            <td><%# FormatDate(Eval("PubDate"))%></td>            
            <td>
            <a href='../Visa/Order_Ok.aspx?id=<%# Eval("id") %>' target="_blank">详情</a>            
			      |
